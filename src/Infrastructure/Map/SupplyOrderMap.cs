...
namespace Infrastructure.Map
{
    public class SupplyOrderMap : IEntityTypeConfiguration<SupplyOrder>
    {
        public void Configure(EntityTypeBuilder<SupplyOrder> builder)
        {
            ...
            builder.Property(so => so.DealerId).IsRequired();
            builder.Property(so => so.SupplyOrderId);
            builder.Property(co => co.TotalAmount);
            builder.HasMany(co => co.SupplyOrderClientOrders).WithOne().HasForeignKey(p => p.SupplyOrderId);
        }
    }
}
